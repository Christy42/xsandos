...
def alphabeta_count(board_arr):
    ...
    for i in range(8):
        for j in range(8):
            count += 3 if board_arr[i][j] == 1 else 5 if board_arr[i][j] == 2 else -3 if board_arr[i][j] == -1 else -5 if board_arr[i][j] == -2
    return count
...
estimators.append(('mlp', KerasRegressor(build_fn=build_model, epochs=50, batch_size=5, verbose=0)))
